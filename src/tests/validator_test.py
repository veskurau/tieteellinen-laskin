...
class TestValidator(unittest.TestCase):
    ...
    def test_token_with_more_than_one_decimal_point_is_rejected(self):
        ...
        returned = self.validator.validate(expression, self.saved_variables, self.all_valid_functions)
    ...
    def test_token_with_decimal_point_must_have_numbers_after_it(self):
        ...
        returned = returned = self.validator.validate(
            expression1, self.saved_variables, self.all_valid_functions)
        self.assertEqual(returned, False)
        returned = returned = self.validator.validate(
            expression2, self.saved_variables, self.all_valid_functions)
        self.assertEqual(returned, False)
        returned = returned = self.validator.validate(
            expression3, self.saved_variables, self.all_valid_functions)
        self.assertEqual(returned, False)
        returned = returned = self.validator.validate(
            expression4, self.saved_variables, self.all_valid_functions)
        self.assertEqual(returned, False)
    ...
    def test_token_with_operation_is_read_correctly(self):
        ...
        for operation in valid_operations:
            ...
            self.validator.validate(operation, self.saved_variables, self.all_valid_functions)
