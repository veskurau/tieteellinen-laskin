...
class TestValidator(unittest.TestCase):
    ...
    def test_token_with_decimal_point_must_have_numbers_after_it(self):
        ...
        expression3 = "15.+"
        expression4 = "15.&&&"
        returned =         returned = self.validator.validate(expression1, self.saved_variables)
        self.assertEqual(returned, False)
        returned =         returned = self.validator.validate(expression2, self.saved_variables)
        self.assertEqual(returned, False)
        returned =         returned = self.validator.validate(expression3, self.saved_variables)
        self.assertEqual(returned, False)
        returned =         returned = self.validator.validate(expression4, self.saved_variables)
        self.assertEqual(returned, False)
